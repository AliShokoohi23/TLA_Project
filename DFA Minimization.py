...
def minimize_dfa(dfa):
    ...
    for min_state in min_states:
        ...
        if min_state in final_states:
            ...
            min_final_states_set.add(min_final_states)
            

        for symbol in input_symbols:
                try:
                    next_state = transitions[min_state][symbol]
                except: 
                    next_state = transitions[min_state[:2]][symbol]
                for partition in partitions:
    ...
    for final_state in min_final_states_set:
        for states in partitions:
            if final_state in states:
                for state in states:
                    min_final_states_set.add(state)
...
file_name = "Minimized DFA.json"
with open(file_name, 'w') as output_file:
        json.dump(minimized_dfa, output_file, indent=2)